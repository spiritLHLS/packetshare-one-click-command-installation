#!/bin/bash
#FROM https://github.com/spiritLHLS/packetshare-one-click-command-installation
...
# 输入 packetshare 的个人信息
input_token(){
  [ -z $EMAIL ] && reading " Enter your Email, if you do not find it, open https://www.packetshare.io/?code=D56687E3945939C7: " EMAIL 
...
container_build(){
  ...
  # 删除旧容器（如有）
  docker ps -a | awk '{print $NF}' | grep -qw "$NAME" && yellow " Remove the old packetshare container.\n " && docker rm -f "$NAME" 
...
uninstall(){
  ...
  docker rmi -f $(docker images | grep packetshare/packetshare:latest | awk '{print $3}')
  green "\n Uninstall containers and images complete.\n"
  exit 0
}

# 传参
while getopts "UuE:e:P:p:" OPTNAME; do
  case "$OPTNAME" in
    'U'|'u' ) uninstall;;
    'E'|'e' ) EMAIL=$OPTARG;;
    'P'|'p' ) PASSWORD=$OPTARG;;
